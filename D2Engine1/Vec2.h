...
public:
	...
	inline _Vec2    Rotation(float angle)
	{
		_Vec2<T> result;
		float c = cosf(angle);
		float s = sinf(angle);
...
typedef _Vec2<float> float2;
typedef _Vec2<UINT>  uint2;
template < typename T >
inline std::vector < _Vec2<T>> CalculateIntersectionPoints(_Vec2<T> q, _Vec2<T> p1, _Vec2<T> p2, T r)
{
	...
	float dr2 = d.LenSq();
	float D = (p1 - q).CrossWith(p2 - q);
	float disc = sq(r) * dr2 - sq(D);
	if (disc >= (T)0.0)
	{
		float sqrtDisc = sqrt(disc);
		float lhsx = D * d.y;
		float rhsx = sgn(d.y)* d.x * sqrtDisc;
		float lhsy = -D * d.x;
		float rhsy = abs(d.y) * sqrtDisc;
